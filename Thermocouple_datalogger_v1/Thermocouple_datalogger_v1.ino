...
int button1INT = 0; //This is the interrupt pin for the control button during the main loop
...
const int chs = 8; //Number of thermocouple channels we'll be reading
const int numReadings = 10; //Number of readings we'll be storing for each channel
long tempInArray[numReadings][chs]; //create array to hold analog values, for 8 channels
int index = 0; //index value to iterate through in main loop for keeping running average
...
void loop(void)
{
  ...
  //first set mux channel to read
    for (int channel = 0; channel < chs; channel++) {
      ...
      delay(5); //delay for 5 milliseconds to give the mux time to settle
  ...
  for (int channel = 0; channel < chs; channel++) {
    ...
    averages[channel] = averages[channel] * aref_voltage / 1024; //convert to voltage
  ...
  int row = 0;
  for (int i = 0; i <=6; i = i + 2) { //this loop runs 4 times, one for each row of LCD
    lcd.setCursor(0,row); //move cursor to next row for subsequent write commands
    lcd.print("Ch");
    lcd.print(i+1); //display channel number (i + 1)
    lcd.print(":");
    lcdPrintDouble(averages[i],1); //see PrintDouble subroutine at bottom of this file
...
//    if(!file.sync()) error("sync"); //file sync command?
    ...
    lcd.print("E");
    delay(400);
    for (int i = 0; i<=3; i++) { //clear save notification
      lcd.setCursor(19,i);
      lcd.print(" ");
    }    
      ...
      if (buttonValue1 != buttonState) { //make sure button state has changed
        if (buttonValue1 == LOW) { //if button is pressed by user (= LOW)
          if(lcdInterval >= 10 && lcdInterval < 30) {
            ...
            lcdInterval = lcdInterval + 180; //set value to 5 minutes (300 seconds)
          } else if(lcdInterval >= 300 && lcdInterval < 600) {
            lcdInterval = lcdInterval + 300; //set value to 10 minutes (600 seconds)
          } else if(lcdInterval >= 600) {
            lcdInterval = 10; //circle back to 10 seconds if value exceeds 600 seconds
          }
