...
void setup(void)
{
  ...
  lcd.setCursor(0,2);
  lcd.print(now.year(),DEC); //display month value
  lcd.print(F("-"));
  lcd.print(now.month(),DEC); //display numeric day value
  lcd.print(F("-"));
  lcd.print(now.day(),DEC); //display year
  delay(3000);
...
byte lcdInt() {
  ...
  unsigned long startTime = now.unixtime();
  lcd.clear();
  lcd.print(F("Choose LCD backlight"));
  lcd.setCursor(0,1);
  lcd.print(F("timeout:"));
  delay(500);
  lcd.setCursor(0,2);
...
void lcdPrintDouble( double val, byte precision){
  ...
  if (int(val) < 100) {
    lcd.print(F(" ")); //pad with a space for values less than 100
    lcd.print(int(val));
  } else {
